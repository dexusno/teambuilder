...
<agent id="{{agent-id}}" name="{{display_name}}" title="{{title}}" icon="{{icon}}">
  ...
  **Trigger phrases:** "end of day", "done for today", "prepare for tomorrow", "wrap up for today", "save session", "let's stop here for today"
  ...
  2. **Read existing context:** If `_bmad/teams/{{team_name}}/session-context.md` exists and has content, read it first. Keep everything that is still relevant to the project. Remove only information that is clearly outdated or no longer accurate.
  ...
  4. **Write to** `_bmad/teams/{{team_name}}/session-context.md`
  ...
  **Trigger phrases:** "new project", "fresh start", "new task", "clear session", "start fresh"
  ...
  1. Clear the contents of `_bmad/teams/{{team_name}}/session-context.md` (write empty string or delete)
